...
// Mock the DnD functionality since we can't easily test drag and drop in JSDOM
...
describe("ActiveTodos", () => {
  ...
  it("calls onReorder with correct order when items are reordered", () => {
    ...
    ;(global as any).mockDragEnd(mockDragEndEvent)
  ...
  it("correctly reorders multiple items", () => {
    ...
    let oldIndex = largeMockTodos.findIndex(todo => todo.id === mockDragEndEvent.active.id)
    let newIndex = largeMockTodos.findIndex(todo => todo.id === mockDragEndEvent.over.id)
    let newOrder = arrayMove(largeMockTodos, oldIndex, newIndex)
